...
class DQNMigrationAlgorithm(Algorithm):
    def __init__(self, agent, reward_giver=None):
        ...
        self.current_machine_service_pairs = []
    ...
    # def __call__(self, clock, mec_net, service=None):
    ...
    #         batch = self.make_input_batch(mec_net, valid_machine_service_pairs)
    #         state = torch.from_numpy(np.array(batch).astype(float)).float()
    ...
    #
    #         dest_machine = valid_machine_service_pairs[action][0]
    #         return dest_machine, state, action, torch.FloatTensor([REWARD])
    #
    #     # When no machines are available to the service for now. Ignore such exceptional cases.
    #     else:
    ...
    def get_state(self, mec_net, service):
        ...
        for machine in machines:
            if self.can_satisfy_e2e_latency(mec_net, service, machine):
                if machine.can_accommodate(service.service_profile):
                    self.current_machine_service_pairs.append((machine, service))
    ...
    def initialize_state_configs(self):
        self.current_machine_service_pairs.clear()
