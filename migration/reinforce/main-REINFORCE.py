...
from migration.reinforce.agent import REINFORCEAgent
from migration.reinforce.algorithm import REINFORCEAlgorithm
from migration.reinforce.reward_giver import LeastAccumPathCostRewardGiver
...
def main():
    ...
    for itr in range(NUM_ITERATIONS):
        ...
        for epi in range(NUM_EPISODES):
            ...
            episode = Episode(None, service_profiles, algorithm)
        ...
        with open("result.txt", 'a') as f:
            print(np.mean(makespans), np.mean(computation_times),
                  ...
                  np.mean(accum_path_costs), np.mean(avg_path_costs), np.mean(avg_num_interruptions),
                  file=f)
